...
def pick(paragraphs, select, k):
    ...
    ''
    """
    picked_paragraphs = [p for p in paragraphs if select(p)]
    if k < len(picked_paragraphs):
...
def about(topic):
    ...
    return lambda paragraph: any([t in split(lower(remove_punctuation(paragraph))) for t in topic])
...
def accuracy(typed, source):
    ...
    stats = [typed_words[i] == source_words[i] if i < len(source_words) else False for i in range(len(typed_words))]
    if len(stats) == 0:
        return 100.0 if len(source_words) == 0 else 0.0
    return 100 * (sum(stats) / len(stats))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    candidate_word = min(word_list, key=lambda word: diff_function(typed_word, word, limit))
    if diff_function(typed_word, candidate_word, limit) > limit:
        return typed_word
...
def feline_fixes(typed, source, limit):
    ...
    5
    """
    def helper(t, s, cur_l):
        if cur_l > limit:
            return cur_l
        if len(t) == 0 or len(s) == 0:
            return len(t) + len(s) + cur_l
        incr = 0 if t[0] == s[0] else 1
        return helper(t[1:], s[1:], cur_l + incr)
...
def time_per_word(words, times_per_player):
    ...
    for time_per_player in times_per_player:
        time = []
        for i in range(1, len(time_per_player)):
            time.append(time_per_player[i] - time_per_player[i - 1])
        times.append(time)
...
def fastest_words(match):
    ...
    selector = lambda word_index: min(player_indices, key=lambda i: (time(match, i, word_index), i))
    res = [list() for _ in player_indices]
    for i in word_indices:
        res[selector(i)].append(get_word(match, i))
    return res
