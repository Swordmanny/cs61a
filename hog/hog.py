...
def roll_dice(num_rolls, dice=six_sided):
    ...
    return 1 if any([roll == 1 for roll in rolls]) else sum(rolls)
...
def perfect_square(score):
    return round(sqrt(score)) * round(sqrt(score)) == score


def next_perfect_square(score):
    return pow(round(sqrt(score) + 1), 2)
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        who_score = score0 * (1 - who) + score1 * who
        opp_score = score0 * who + score1 * (1 - who)
        strategy = strategy0 if who == 0 else strategy1
        updated_score = update(strategy(who_score, opp_score), who_score, opp_score, dice)
        if who == 0:
            score0 = updated_score
        else:
            score1 = updated_score
        who = 1 - who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal):
        for opponent_score in range(goal):
            prev = ans
            ans = strategy(score, opponent_score)
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    def averaged(*args):
        return sum([original_function(*args) for i in range(total_samples)]) / total_samples
    return averaged
