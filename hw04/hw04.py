...
def balanced(m):
    ...
    True
    """
    assert is_mobile(m), 'm has to be a mobile'
    return length(left(m)) * total_weight(end(left(m))) == \
           length(right(m)) * total_weight(end(right(m))) and \
           (balanced(end(left(m))) if is_mobile(end(left(m))) else True) and \
           (balanced(end(right(m))) if is_mobile(end(right(m))) else True)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    return add_interval(x, interval(-upper_bound(y), -lower_bound(x)))
...
def div_interval(x, y):
    ...
    assert lower_bound(x) * upper_bound(y) > 0
